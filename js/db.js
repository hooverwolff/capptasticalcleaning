...
async function ccUpsertDiscount(discount) {
  ...
  let error;
  if (discount.id) {
    ({ error } = await supabaseClient.from('discount_codes').update(row).eq('id', discount.id));
  } else {
    ({ error } = await supabaseClient.from('discount_codes').insert(row));
  }
...
async function ccValidateDiscount(code, dateStr) {
  const { data, error } = await supabaseClient.rpc('validate_and_apply_discount', {
...
async function ccGetBlackouts() {
  const { data, error } = await supabaseClient.from('blackout_dates').select('*').order('date', { ascending: true });
...
async function ccUpsertBlackout(b) {
  ...
  let error;
  if (b.id) {
    ({ error } = await supabaseClient.from('blackout_dates').update(row).eq('id', b.id));
  } else {
    ({ error } = await supabaseClient.from('blackout_dates').insert(row));
  }
  ccThrowIfError(error, 'saving blocked time');
}

async function ccDeleteBlackout(id) {
  const { error } = await supabaseClient.from('blackout_dates').delete().eq('id', id);
...
async function ccGetTakenSlots(dateStr) {
  const { data, error } = await supabaseClient.rpc('get_taken_slots', { p_date: dateStr });
...
async function ccGetAdmins() {
  const { data, error } = await supabaseClient.from('admin_users').select('*').order('created_at', { ascending: true });
  ccThrowIfError(error, 'loading staff list');
  return data;
}

async function ccAddAdmin(email, displayName) {
  const { error } = await supabaseClient.from('admin_users').insert({ email: email.trim().toLowerCase(), display_name: displayName || null });
